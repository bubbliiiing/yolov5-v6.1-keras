...
def SPPFBottleneck(x, out_channels, name = ""):
    #---------------------------------------------------#
...
def resblock_body(x, num_filters, num_blocks, expansion=0.5, shortcut=True, last=False, name = ""):
    ...
    x = C3(x, num_filters, num_blocks, shortcut=shortcut, expansion=expansion, name = name + '.1')
    if last:
        x = SPPFBottleneck(x, num_filters, name = name + '.2')
    return x
...
def darknet_body(x, base_channels, base_depth):
    ...
    x = DarknetConv2D_BN_SiLU(base_channels, (6, 6), strides = (2, 2), name = 'backbone.stem')(x)
    
    # 320, 320, 64 => 160, 160, 128
    x = resblock_body(x, base_channels * 2, base_depth, name = 'backbone.dark2')
    
    # 160, 160, 128 => 80, 80, 256
    x = resblock_body(x, base_channels * 4, base_depth * 2, name = 'backbone.dark3')
    feat1 = x
